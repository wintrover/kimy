...
The `{{contract}}` placeholder, if present in the prompt_template, is replaced with the active swarm contract content at spawn time. The contract is an optional orchestration-level agreement (acceptance criteria, constraints, scope) injected into every subagent prompt. Set it via `setSwarmContract()` before launching the swarm; if no contract is active the placeholder is removed silently.
